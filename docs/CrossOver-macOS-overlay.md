...
1. **Patch the Mac driver** — from the EQBuddy repo:
   ...
   ```
   It rebuilds only `winemac.so` (or uses a committed prebuilt), backs up your
   original, and installs it. Needs Xcode command-line tools and Homebrew
   `bison`. Undo any time with `--revert`.
...
- **CrossOver updates overwrite `winemac.so`** — re-run `setup-overlay.sh` after
  each update. After a *major* Wine version bump, delete any stale prebuilt so it
  rebuilds from the new matching source (an old binary could be ABI-incompatible).
